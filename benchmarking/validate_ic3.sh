#!/bin/sh
# Validate --ic3 verdicts against expected results in hwmcc08results.csv
# (column 3, abc's verdict: "uns" = proved, "sat" = refuted).
# Usage: validate_ic3.sh <timeout-seconds> <benchmark>...
...
for b in "$@"; do
  ...
  out=`perl -e 'alarm shift @ARGV; exec @ARGV' $TIMEOUT $EBMC "$f" --ic3 2>&1`
