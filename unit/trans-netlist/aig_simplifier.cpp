...
SCENARIO("AIG simplification with equivalences")
{
  ...
  GIVEN("An AIG where a variable is equivalent to true")
  {
    aigt aig;
    auto v0 = aig.new_input();      // node 0
    auto v1 = aig.new_input();      // node 1
    ...
    // v0 is equivalent to true
    equivalencest equivalences = {{v0, const_literal(true)}};

    THEN("The AND simplifies to v1")
    {
      auto [result, _] = aig_simplify(aig, equivalences);
      // v0 AND v1 becomes true AND v1 = v1
      // Only v1's representative is created
      REQUIRE(result.number_of_nodes() == 1);
      REQUIRE(result.nodes[0].is_input());
    }
  }

  GIVEN("An AIG where a variable is equivalent to false")
  {
    aigt aig;
    auto v0 = aig.new_input();      // node 0
    auto v1 = aig.new_input();      // node 1
    ...
    // v0 is equivalent to false
    equivalencest equivalences = {{v0, const_literal(false)}};

    THEN("The AND simplifies to false")
    {
      auto [result, _] = aig_simplify(aig, equivalences);
      // v0 AND v1 becomes false AND v1 = false (constant)
      // Only v1's representative is created
      REQUIRE(result.number_of_nodes() == 1);
      REQUIRE(result.nodes[0].is_input());
    }
  ...
  GIVEN("A deeper AIG with equivalence collapsing a subtree")
  {
    aigt aig;
    auto v0 = aig.new_input();            // node 0
    auto v1 = aig.new_input();            // node 1
    ...
    // v2 ≡ false, which should make and2 = false, and then the top = false
    equivalencest equivalences = {{v2, const_literal(false)}};

    THEN("The false propagates through the tree")
    {
      auto [result, _] = aig_simplify(aig, equivalences);
      // v2 = false makes (v2 AND v3) = false
      // Then (v0 AND v1) AND false = false
      // We still have v0, v1, v3 as variables, plus (v0 AND v1)
      REQUIRE(result.number_of_nodes() == 4);
      REQUIRE(result.nodes[0].is_input());
      REQUIRE(result.nodes[1].is_input());
      REQUIRE(result.nodes[2].is_input());
      // The AND node: v0 AND v1
      REQUIRE(result.nodes[3].is_and());
      REQUIRE(result.nodes[3].a == literalt{0, false});
      REQUIRE(result.nodes[3].b == literalt{1, false});
    }
...
SCENARIO("AIG simplification preserves and re-numbers labeling")
{
  ...
  GIVEN("An AIG with a labeled input equivalent to a constant")
  {
    aigt aig;
    auto v0 = aig.new_input("input_a"); // node 0
    auto v1 = aig.new_input("input_b"); // node 1
    ...
    // v1 ≡ true
    equivalencest equivalences = {{v1, const_literal(true)}};

    THEN("Label is re-numbered to the constant")
    {
      auto [result, _] = aig_simplify(aig, equivalences);
      // v1 becomes true, AND(v0, true) = v0
      REQUIRE(result.number_of_nodes() == 1);
      REQUIRE(result.labeling.size() == 2);
      REQUIRE(result.labeling.at("input_a") == literalt{0, false});
      REQUIRE(result.labeling.at("input_b") == const_literal(true));
    }
  }
  ...
  GIVEN(
    "An AIG where the AND node output is set to false but inputs are "
    "unconstrained")
  {
    aigt aig;
    auto v0 = aig.new_input();                // node 0
    auto v1 = aig.new_input();                // node 1
    auto and_node = aig.new_and_node(v0, v1); // node 2: v0 AND v1

    // Only the AND node output is equivalent to false
    // The inputs v0 and v1 are not constrained initially
    equivalencest equivalences = {{and_node, const_literal(false)}};

    THEN("The inputs also simplify to false via backward propagation")
    {
      // Get both the result and the substitution map
      auto [result, substitution] = aig_simplify(aig, equivalences);
      REQUIRE(substitution.size() == 3);

      // Check that v0 (node 0) is substituted to false
      REQUIRE(substitution[0].is_false());

      // Check that v1 (node 1) is substituted to false
      REQUIRE(substitution[1].is_false());

      // Check that the AND node (node 2) is substituted to false
      REQUIRE(substitution[2].is_false());

      // When AND(v0, v1) = false, backward propagation deduces
      // that both v0 and v1 must be false, so result is empty
      REQUIRE(result.number_of_nodes() == 0);
    }
  }

  GIVEN("An AIG where an AND node with negated inputs is set to false")
  {
    aigt aig;
    auto v0 = aig.new_input();                  // node 0
    auto v1 = aig.new_input();                  // node 1
    ...
    // Set the AND node output to false
    equivalencest equivalences = {{and_node, const_literal(false)}};

    THEN("Backward propagation sets inputs to true so negations become false")
    {
      // Get both the result and the substitution map
      auto [result, substitution] = aig_simplify(aig, equivalences);
      REQUIRE(substitution.size() == 3);

      // !v0 AND !v1 = false means both !v0 and !v1 must be false
      // Therefore v0 must be true and v1 must be true
      REQUIRE(substitution[0].is_true());
      REQUIRE(substitution[1].is_true());
      REQUIRE(substitution[2].is_false());

      // Both inputs become constants (true), so result is empty
      REQUIRE(result.number_of_nodes() == 0);
    }
  }

  GIVEN("An AIG where an AND node with mixed negated inputs is set to false")
  {
    aigt aig;
    auto v0 = aig.new_input();                 // node 0
    auto v1 = aig.new_input();                 // node 1
    ...
    THEN("Backward propagation correctly handles mixed negations")
    {
      // Get both the result and the substitution map
      auto [result, substitution] = aig_simplify(aig, equivalences);
      REQUIRE(substitution.size() == 3);
      ...
      // !v0 AND v1 = false means both !v0 and v1 must be false
      // Therefore v0 must be true and v1 must be false
      REQUIRE(substitution[0].is_true());  // v0 = true (so !v0 = false)
      REQUIRE(substitution[1].is_false()); // v1 = false
      REQUIRE(substitution[2].is_false()); // AND node = false

      // Both inputs become constants, so result is empty
      REQUIRE(result.number_of_nodes() == 0);
    }
  }

  GIVEN("An AIG with a chain of AND nodes where the final output is false")
  {
    aigt aig;
    auto v0 = aig.new_input();              // node 0
    auto v1 = aig.new_input();              // node 1
    auto v2 = aig.new_input();              // node 2
    auto and1 = aig.new_and_node(v0, v1);   // node 3: v0 AND v1
    auto and2 = aig.new_and_node(and1, v2); // node 4: (v0 AND v1) AND v2

    // Set the final AND node output to false
    equivalencest equivalences = {{and2, const_literal(false)}};

    THEN("Backward propagation sets all inputs to false")
    {
      // Get both the result and the substitution map
      auto [result, substitution] = aig_simplify(aig, equivalences);
      REQUIRE(substitution.size() == 5);

      // All inputs should be false
      REQUIRE(substitution[0].is_false()); // v0 = false
      REQUIRE(substitution[1].is_false()); // v1 = false
      REQUIRE(substitution[2].is_false()); // v2 = false
      REQUIRE(substitution[3].is_false()); // and1 = false
      REQUIRE(substitution[4].is_false()); // and2 = false

      // All nodes become constants, so result is empty
      REQUIRE(result.number_of_nodes() == 0);
    }
  }
}
