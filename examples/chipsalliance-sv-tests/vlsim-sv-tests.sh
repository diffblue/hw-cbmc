...
# copy the vlsim runner, removing others to avoid dependency issues
rm -f sv-tests/tools/runners/*.py
cp "$SCRIPT_DIR/vlsim.py" sv-tests/tools/runners/

cd sv-tests

# run the sv-tests with vlsim only and generate the HTML report
make RUNNERS=vlsim report

# print a summary of pass/fail results
python3 - <<'EOF'
import os, sys

logs_dir = 'out/logs/vlsim'
...
print(f'\nvlsim: {len(passed)} passed, {len(failed)} failed out of {total} tests')
EOF
