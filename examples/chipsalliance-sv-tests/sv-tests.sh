...
if [ ! -e sv-tests/.git ] ; then
  ...
  # Pin to a fixed commit for reproducible CI runs: upstream has not cut
  # a release or tag since 2019, so a commit hash is the only stable
  # anchor available.  This is the tip of 'main' as of 2025-01-07.  When
  # bumping this hash, run this script locally first and compare the
  # ebmc pass/fail summary (printed below) against the previous commit
  # to catch regressions caused by upstream test changes.
...
# sv-tests' Makefile auto-discovers every *.py file placed under
# tools/runners/ as a runner (see RUNNERS_FOUND in tools/runners.mk), so
# copying our runner there is sufficient for 'make report' below to pick
# it up; no further registration is needed.
