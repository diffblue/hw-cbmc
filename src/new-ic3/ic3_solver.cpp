  ...
  : message_handler(message_handler)
{
  ...
  {
    // We ask unwind() not to assert the in-state AIG constraints.
    ...
    // The transition relation T is assumed to the left-total, which
    // avoids that a state in s with no successor under i satisfies
    // s ∧ i ∧ T ⇒ cube' vacuously.
    messaget message{message_handler};
    unwind_optionst unwind_options;
    unwind_options.add_initial_state = false;
    unwind_options.add_constraints = false; // see comment above
    ::unwind(netlist, bmc_map, message, *base_cnf, unwind_options, 0);
  }
