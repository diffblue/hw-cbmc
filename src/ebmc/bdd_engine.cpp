  ...
  for(auto &property : properties.properties)
    if(property.is_assumed())
    {
        ...
        !has_temporal_operator(to_unary_expr(expr).op()))
      {
        ...
        CHECK_RETURN(netlist_property != netlist.properties.end());
        DATA_INVARIANT(
          netlist_property->second.id() == ID_sva_assume,
          "assumed property must be sva_assume");
        auto &assumption = to_sva_assume_expr(netlist_property->second).op();
        DATA_INVARIANT(
          assumption.id() == ID_sva_always,
          "assumed property must be sva_assume sva_assert");
        auto &p = to_sva_always_expr(assumption).op();
        DATA_INVARIANT(
          p.id() == ID_literal,
