...
{
  status("Solving with "+solver.decision_procedure_text());
  ...
  status("Solver time: "+time2string(
    current_time()-sat_start_time));
...
int ebmc_baset::finish(const bmc_mapt &bmc_map, propt &solver)
{
  ...
  status("Solving with "+solver.solver_text());
...
int ebmc_baset::get_model()
{
  ...
  if(cmdline.isset("smv-netlist"))
  {
    netlistt netlist;
    make_netlist(netlist);
    std::cout << "-- Generated by EBMC " << EBMC_VERSION << std::endl;
    std::cout << "-- Generated from " << main_symbol->name << std::endl;
    std::cout << std::endl;
...
void ebmc_baset::show_ldg(std::ostream &out)
{
  ...
  if(!netlist.transition.empty())
    out << "WARNING: transition constraint found!" << std::endl
        << std::endl;
  ...
  out << "Latch dependencies:" << std::endl;
