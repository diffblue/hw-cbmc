...
{
  PRECONDITION(curr.operands().size() == 1);
  PRECONDITION(next.operands().size() == 1);
  ...
  const decision_proceduret &solver) const
{
  ...
  for(auto &weight : net.fc1.neurons[0].weights)
    std::cout << "weight: " << format(weight) << " = "
              << format(solver.get(weight)) << '\n';
...
exprt solver_neural_livenesst::guess(propertyt &property)
{
  ...
  auto net = ranking_nett{state_variable_map.size(), net_type};
...
int do_solver_neural_liveness(
  const cmdlinet &cmdline,
  ui_message_handlert &ui_message_handler)
{
  return solver_neural_livenesst(cmdline, ui_message_handler)();
}
