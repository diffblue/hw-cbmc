...
{
  dimacs_cnft dimacs_cnf{message.get_message_handler()};

  int result = do_bit_level_bmc(dimacs_cnf, true);
  if(result!=0) return result;

  message.statistics() << dimacs_cnf.no_variables() << " variables and "
                       << dimacs_cnf.no_clauses() << " clauses"
                       << messaget::eom;
...
int ebmc_baset::do_sat()
{
  ...
  {
    const namespacet ns(transition_system.symbol_table);
    boolbvt boolbv(ns, satcheck, message.get_message_handler());
...
#else
  message.error() << "Support for prover not linked in" << messaget::eom;
  return 1;
#endif
}
...
#else
  message.error() << "Support for lifter not linked in" << messaget::eom;
  return 1;
#endif
}
