...
{
  if(sequence_expr.id() == ID_sva_sequence_concatenation)
  {
    ...
    auto rhs_rec =
      sva_sequence_monitor_rec(transition_system, lhs_rec.done(), rhs);
    if(rhs_rec.is_error())
      return rhs_rec;
  ...
  const exprt &property_expr)
{
  ...
  else if(property_expr.id() == ID_sva_sequence_property)
  {
    return sva_sequence_monitor_rec(
      transition_system,
      active,
      to_sva_sequence_property_expr(property_expr).op());
  }
  else
    return monitor_resultt::error(
