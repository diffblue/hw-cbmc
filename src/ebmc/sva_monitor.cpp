...
public:
  ...
  // factory for the 'safe' and 'done' case
  static monitor_resultt safe_and_done(exprt __safe, exprt __done)
  {
    monitor_resultt monitor_result;
    monitor_result._safe = std::move(__safe);
    monitor_result._done = std::move(__done);
    return monitor_result;
  }

  const exprt &done() const
  {
    return _done;
  }
...
monitor_resultt sva_monitor_rec(
  ...
  const exprt &property_expr)
{
  if(!has_temporal_operator(property_expr))
  {
    ...
    // It is 'done' once 'active'.
    return monitor_resultt::safe_and_done(
      implies_exprt{active, property_expr}, active);
  }
  else if(property_expr.id() == ID_not)
  {
    ...
    if(nnf_opt)
      return sva_monitor_rec(transition_system, active, *nnf_opt);
    else
  ...
  else if(property_expr.id() == ID_and)
  {
    ...
    {
      auto rec = sva_monitor_rec(transition_system, active, op);
      if(rec.is_error())
        return rec;
  ...
  else if(property_expr.id() == ID_sva_always)
  {
    ...
    return sva_monitor_rec(transition_system, always_active_expr, op);
  }
  else if(property_expr.id() == ID_sva_s_always)
  {
      ...
      numeric_cast_v<mp_integer>(to_constant_expr(s_always_expr.upper())) == 1)
    {
      ...
      return sva_monitor_rec(
        transition_system, delayed_active_expr, s_always_expr.op());
    }
    else
      return monitor_resultt::error("s_always is TBD");
  }
  else if(property_expr.id() == ID_sva_sequence_concatenation)
  {
    auto &concatenation = to_sva_sequence_concatenation_expr(property_expr);
    ...
    // 3. The overall 'done' signal is the rhs done signal.
    auto lhs_rec = sva_monitor_rec(transition_system, active, lhs);
    if(lhs_rec.is_error())
      return lhs_rec;

    auto rhs_rec = sva_monitor_rec(transition_system, lhs_rec.done(), rhs);
    if(rhs_rec.is_error())
      return rhs_rec;

    return monitor_resultt::safe_and_done(
      and_exprt{lhs_rec.safe(), rhs_rec.safe()}, rhs_rec.done());
  }
  else if(property_expr.id() == ID_sva_cycle_delay)
  {
    auto &delay = to_sva_cycle_delay_expr(property_expr);
  ...
  ebmc_propertiest::propertyt &property)
{
  ...
  auto result =
    sva_monitor_rec(transition_system, start, property.normalized_expr);
