...
// Ripple-carry adder delay, shared by plus/minus and unary minus (a
// two's-complement negation is structurally one adder). Generic
// synthesis measured as linear in width: real delay went
// 0.24/0.54/1.11/2.18/4.30 ns for n=8/16/32/64/128, i.e. ~34 ps/bit
// with negligible fixed overhead.
static constexpr double ADDER_DELAY_PS_PER_BIT = 34.0;
  ...
  : process(process_name)
{
  ...
  if(it == process_table.end())
    throw ebmc_errort{}.with_exit_code(CPROVER_EXIT_USAGE_ERROR)
      << "unknown process node: " << process_name;
...
technology_dbt::operator_cost(const irep_idt &op, std::size_t width) const
{
  ...
  {
    area_per_bit = 6.0;
    delay = ADDER_DELAY_PS_PER_BIT * n;
    energy_per_bit = 3.0;
  }
  else if(op == ID_mult)
  {
  ...
  // MUX / if-then-else. The effective calibrated cost is 2.1 um2/bit:
  // this used to be a 3.0 um2/bit base cost to which the generic
  // multi-input-gate correction (0.7, meant for wide AND/OR gates) was
  // unconditionally applied, since every if-then-else has three operands.
  // The correction no longer applies to muxes, so the factor is folded
  // into the base cost to keep the calibrated totals unchanged.
  ...
  // Unary minus (two's complement): structurally one adder.
