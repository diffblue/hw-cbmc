...
tvt aig_prop_solvert::l_get(literalt a) const { return solver.l_get(a); }

propt::resultt aig_prop_solvert::prop_solve() {
  log.status() << "converting AIG, " << aig.nodes.size() << " nodes"
               << messaget::eom;
  convert_aig();
...
void aig_prop_solvert::compute_phase(std::vector<bool> &n_pos,
  ...
  for (aig_plus_constraintst::constraintst::const_iterator c_it =
           aig.constraints.begin();
       c_it != aig.constraints.end(); c_it++)
    queue.push(*c_it);
  ...
  log.statistics() << "Pos only: " << pos_only << "\n"
                   << "Neg only: " << neg_only << "\n"
                   << "Mixed: " << mixed << messaget::eom;
}
...
void aig_prop_solvert::usage_count(std::vector<unsigned> &p_usage_count,
  ...
  log.statistics() << "Unused: " << unused << " "
                   << "Used once: " << usedOncePositive + usedOnceNegative
                   << " (P: " << usedOncePositive << ", N: " << usedOnceNegative
                   << ") "
                   << "Used twice: "
                   << usedTwicePositive + usedTwiceNegative + usedTwiceMixed
                   << " (P: " << usedTwicePositive
                   << ", N: " << usedTwiceNegative << ", M: " << usedTwiceMixed
                   << ") "
                   << "Used three times: " << usedThreeTimes << " "
                   << "Used more: " << usedMore << messaget::eom;
#endif
}
...
void aig_prop_solvert::convert_node(unsigned n, const aigt::nodet &node,
  ...
  if (p_usage_count[n] > 0 || n_usage_count[n] > 0) {
    ...
    // by the CNF encoding
    if (body.size() == 3 && body[0].sign() && body[1].sign() &&
        body[2].sign()) {
      const aigt::nodet &left = aig.nodes[body[0].var_no()];
...
void aig_prop_solvert::convert_aig() {
  ...
  for (std::size_t n = aig.nodes.size() - 1; n != 0; n--) {
    if (aig.nodes[n].is_and()) {
#ifdef USE_PG
      convert_node(n, aig.nodes[n], n_pos[n], n_neg[n], p_usage_count,
                   n_usage_count);
#else
