...
public:
  ...
  int max_num_vars; // max_num_vars0 plus variables that take into account 
                    // that property is specified in terms of present and next 
                    //state variables
  ...
  std::vector <TimeFrame> Time_frames; //Time_frames[i] specifies data members
                                       //of  i-th time frame
  std::vector <ClauseInfo> Clause_info; //Clause_info[i] gives information about
  // cube F[i]

  ClauseTable Clause_table; // used to eliminate duplicates

  int tf_lind; //(lind stands for Largest IND) specifies the value of the latest
               // time frame for which  an approximation is built
  ...
  float max_act_val; // if the value of 'max_act_val' has exceeded the value 
                     //of 'factor', it is scaled down
  int cut_off_tf; // used to mix RAND_LIT and other strategies
  ...
  void print_header();
  int mic3();
  void read_input(char *fname,char file_type);
  ...
  void read_parameters(int argc,char *argv[],char file_type);
  ...
  // #include "m3ethods.hh"  // debugging methods

protected:

  // ------------------ Statistics
  int max_num_tfs; // specifies the number of time frames mic3 had to 
                   // use to solve the formula
  int orig_ind_cls; // gives the total number of original 
                    // inductive clauses
  int succ_impr;  // gives the total number of successful improvements
                  //  of inductive clauses
  int failed_impr; // failed improvements
  int max_num_impr; // specifies the maximum number of improvements
                    //  for an inductive clause
  int num_push_clause_calls; // contains the number of sat calls 
  // to push clauses
  int num_inact_cls; // specifies the number of clauses of F that
  // are currently inactive
  int num_add1_cases; // number of cases where 'replace_or_add_clause' ret. ADD1
