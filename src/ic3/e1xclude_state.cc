...
void CompInfo::exclude_state_cube(CNF &G,int &min_tf,CUBE &St0_cube,CUBE &Inps0)
{
  ...
  while (Pr_queue.size() > 0) {
    ...
    bool state_found = find_prev_state_cube(C,curr_tf,Erl_st_cube,
					    Erl_inps,St_cube);
    if (state_found == 0) {
      p();
...
bool CompInfo::gen_ind_clause(CLAUSE &Res,CUBE &St,int tf_ind,char st_descr)
{
  ...
  bool found = lngst_ind_clause(C1,Slvr,C0,st_descr);
  ...
  shorten_clause(Res,tf_ind,C1,st_descr);
