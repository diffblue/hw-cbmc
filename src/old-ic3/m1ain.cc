...
/*=====================
  ...
  ====================*/
property_checker_resultt old_ic3_engine(
...
} /* end of function old_ic3_engine */

bool old_ic3_supports_property(const exprt &expr)
{
...

{
  ...
  try    {
    ...
    for(auto &property : properties.properties)
    {
      ...
      if(!old_ic3_supports_property(property.normalized_expr))
      {
        property.failure("property not supported by old IC3 engine");
        continue;
      }
    ...
    if(netlist.var_map.latches.empty())
    {
      for(auto &property : properties.properties)
        if(property.is_unknown())
          property.failure("no latches -- not supported by the old IC3 engine");
      return property_checker_resultt{properties};
    }
